...
class BaseTransformer(pl.LightningModule):
    ...
    def get_lr_scheduler(self, optimizer: torch.optim.Optimizer, frequency=1):
        ...
        if self.hparams.lr_scheduler in GET_SCHEDULER_FUNCS:
            ...
            scheduler = get_schedule_func(
                optimizer, num_warmup_steps=self.hparams.warmup_steps, num_training_steps=self.training_total_steps()
            )
    ...
    def training_total_steps(self) -> int:
        ...
        return (self.training_dataset_size / effective_batch_size) * self.hparams.max_epochs

    def setup(self, stage: Optional[str] = None):
        # stage: train, test, eval
        self.training_dataset_size = len(self.train_dataloader().dataset)
